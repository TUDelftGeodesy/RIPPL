...
try:
    opts, args = getopt.getopt(sys.argv[:1], "s:e:", ["start_date=", "end_date="])
except getopt.GetoptError:
    print('missing input dates for processing -s <start_date (yyyymmdd)> -e <end_date (yyyymmdd)>')
    sys.exit(2)
for opt, arg in opts:
    if opt in ("-s", "--start_date"):
        start_date = datetime.datetime.strptime(arg, '%Y%m%d')
        print('start date is ' + start_date.date())
    elif opt in ("-e", "--end_date"):
        end_date = datetime.datetime.strptime(arg, '%Y%m%d')
        print('start date is ' + end_date.date())
