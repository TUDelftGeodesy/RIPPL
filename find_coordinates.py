...
class FindCoordinates():
    ...
    @staticmethod
    def interval_sparse_coors(in_shape, s_lin=0, s_pix=0, lines=0, multilook='', oversample='', offset='', factor=''):

        if len(factor) == 0:
            factor = [1, 1]

        sample, multilook, oversample, offset, [lines, pixels] = \
            FindCoordinates.interval_lines(in_shape, s_lin, s_pix, lines, multilook, oversample, offset)

        # First define the first line by an offset of the multilooking window
        offset = offset - multilook
        s_lin = lines[0] - multilook[0]
        s_pix = pixels[0] - multilook[1]

        # Change multilooking window by the given factors
        multilook = multilook * np.array(factor)

        # Find new image size. Should extend outside the original one to be sure interpolation is always possible.
        shape = [int((lines[-1] - s_lin) / (multilook[0] / oversample[0])) + 1,
                 int((pixels[-1] - s_pix) / (multilook[1] / oversample[1])) + 1]
    ...
    @staticmethod
    def interval_sparse_lines(in_shape, s_lin=0, s_pix=0, lines=0, multilook='', oversample='', offset='', factor=''):

        sample, multilook, oversample, offset, [s_lin, s_pix, shape] = \
            FindCoordinates.interval_sparse_coors(in_shape, s_lin, s_pix, lines, multilook, oversample, offset, factor)

        lines = s_lin + np.arange(shape[0]) * (multilook[0] / oversample[0])
        pixels = s_pix + np.arange(shape[1]) * (multilook[1] / oversample[1])
