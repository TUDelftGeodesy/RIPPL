...
"""
In this notebook we will explain the RIPPL workflow based on a study case about the Patagonian ice fields.
...
patagonia_shape = [(-74.091,-47.897), (-72.576,-47.906), (-72.514,-52.538), (-74.153,-52.333), (-74.091,-47.897)]
study_area = ReadWriteShapes()
study_area(patagonia_shape)
...
search_date = datetime.datetime(year=2019, month=5, day=4, hour=22)
time_window = datetime.timedelta(days=50)

find_track = DownloadSentinel(date=search_date, time_window=time_window,
...
reference_date = datetime.datetime(year=2019, month=5, day=26)
...
max_height = 6000       # Expected maximum elevation in area of interest
dem_type = 'TDM30'      # DEM type of data we download (SRTM1, SRTM3, TDM30 and TDM90 are supported)
...
s1_processing.geocode_calc_geometry_multilooked(ml_name='geographic_100m', dem_type=dem_type)
...
# The actual creation of the calibrated amplitude images
s1_processing.create_ml_coordinates(name='mercator_250m', coor_type='projection', oblique_mercator=True, dx=dx, dy=dy, buffer=dem_buffer, rounding=dem_rounding, min_height=min_height, max_height=max_height, overwrite=True)
# To calculate the calibrated amplitudes we need the incidence angle.
s1_processing.geocode_calc_geometry_multilooked(ml_name='mercator_250m', dem_type=dem_type)
s1_processing.calc_calibrated_amplitude(polarisation=polarisation, ml_name='mercator_250m')

# Create the output geotiff files
s1_processing.create_output_geotiffs('calibrated_amplitude', ml_name='mercator_250m')
...
s1_processing.calc_interferogram_coherence(polarisation=polarisation, ml_name='mercator_250m')
s1_processing.calc_interferogram_coherence(polarisation=polarisation, ml_name='geographic_100m')

# Create output geotiffs
s1_processing.create_output_geotiffs('coherence', ml_name='mercator_250m')
s1_processing.create_output_geotiffs('interferogram', ml_name='mercator_250m')
...
s1_processing.unwrap(polarisation, ml_name='mercator_250m')
s1_processing.create_output_geotiffs('unwrap', ml_name='mercator_250m')
...
for ml_name in ['geographic_100m']:
