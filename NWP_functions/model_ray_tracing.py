    ...
    def calc_cross_sections(self):
        ...
        for t in self.t:
            ...
            for l in range(self.lev):
                self.slice_delays['total'][t][l, :, :] = Resample.resample_grid(res_dummy, self.delay_data[t]['total_delay'][l, :, :],
                                                              lat_id, lon_id, 'linear', table_size=[50, 50])
                if self.split_signal:
                    self.slice_delays['wet'][t][l, :, :] = Resample.resample_grid(res_dummy, self.delay_data[t]['wet_delay'][l, :, :],
                                                                  lat_id, lon_id, 'linear', table_size=[50, 50])
                    self.slice_delays['hydrostatic'][t][l, :, :] = Resample.resample_grid(res_dummy, self.delay_data[t]['hydrostatic_delay'][l, :, :],
                                                                  lat_id, lon_id, 'linear', table_size=[50, 50])
                    self.slice_delays['liquid'][t][l, :, :] = Resample.resample_grid(res_dummy, self.delay_data[t]['liquid_delay'][l, :, :],
                                                                  lat_id, lon_id, 'linear', table_size=[50, 50])

            # Also find the heights at these points.
            for l in range(self.lev + 1):
                self.slice_heights[t][l, :, :] = Resample.resample_grid(res_dummy, self.delay_data[t]['heights'][l, :, :],
                                                     lat_id, lon_id, 'linear', table_size=[50, 50])
