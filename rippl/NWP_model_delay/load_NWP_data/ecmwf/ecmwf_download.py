...
class CDSdownload:
    ...
    def check_overpass_available(self, overpass_time):
        ...
        if self.fc_step:
            data_time = ECMWFDatabase.find_closest_dataset(overpass_time, interval_hours=self.fc_step)
            interval = self.fc_step
        else:
            data_time = ECMWFDatabase.find_closest_dataset(overpass_time, interval_hours=self.an_step)
    ...
    def prepare_download(self):
        ...
        for overpass_time in self.overpass_times:
            ...
            before = ECMWFDatabase.find_closest_dataset(overpass_time, interval_hours=self.an_step, date_type='before')
            after = ECMWFDatabase.find_closest_dataset(overpass_time, interval_hours=self.an_step, date_type='after')
            if self.fc_step:
                before_fc = ECMWFDatabase.find_closest_dataset(overpass_time, interval_hours=self.fc_step, date_type='before')
                after_fc = ECMWFDatabase.find_closest_dataset(overpass_time, interval_hours=self.fc_step, date_type='after')
