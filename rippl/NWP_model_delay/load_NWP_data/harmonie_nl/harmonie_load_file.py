...
class HarmonieData(object):
    ...
    def load_harmonie(self, time, filename=''):
        ...
        if not filename:
            filename, time = self.database(time)
        ...
        dat_types = ['U component of wind', 'V component of wind', 'Temperature', 'Specific humidity',
                     'Specific cloud liquid water content', 'Specific cloud ice water content']
        ...
        if not filename in self.filenames:
            ...
            for dat_type, dat_num, dat_name in zip(dat_types, dat_nums, dat_names):
                try:
                    ...
                    # Maybe needed for new harmonie cycle
                ...
                # Flip the data to start with the lowest layer
            ...
            a = np.flip(np.array(pressure.attrs['GRIB_pv'][:levels + 1]))
            b = np.flip(np.array(pressure.attrs['GRIB_pv'][levels + 1:]))
