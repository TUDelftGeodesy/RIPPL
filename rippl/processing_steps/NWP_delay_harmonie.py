...
class HarmonieAPS(Process):  # Change this name to the one of your processing step.
    ...
    def __init__(self, data_id='', in_coor=[], out_coor=[], correct_time_delay=True, time_delays=[], split_signal=False,
                 secondary_slc='secondary_slc', reference_slc='reference_slc', overwrite=False, reference_crop=True, nwp_model_data=''):
        ...
        self.output_info['file_names'] = ['harmonie_aps', 'harmonie_geometry_corrected_aps']
        ...
        if correct_time_delay:
            self.output_info['file_names'].append('harmonie_time_corrected_aps')
            self.output_info['file_names'].append('harmonie_time_geometry_corrected_aps')
            self.output_info['data_types'].extend(['real4', 'real4'])
            for time_delay in time_delays:
                self.output_info['file_names'].append('harmonie_time_corrected_aps_' + str(time_delay) + '_minutes')
                self.output_info['file_names'].append('harmonie_time_geometry_corrected_aps_' + str(time_delay) + '_minutes')
                self.output_info['data_types'].extend(['real4', 'real4'])
        else:
            for time_delay in time_delays:
                self.output_info['file_names'].append('harmonie_aps_' + str(time_delay) + '_minutes')
                self.output_info['file_names'].append('harmonie_geometry_corrected_aps_' + str(time_delay) + '_minutes')
                self.output_info['data_types'].extend(['real4', 'real4'])

        # Input data information
        self.input_info = dict()
        self.input_info['image_types'] = ['reference_slc', 'reference_slc', 'reference_slc', 'reference_slc', 'reference_slc', 'reference_slc']
        self.input_info['process_names'] = ['geocode', 'geocode', 'radar_geometry', 'radar_geometry', 'radar_geometry', 'dem']
        ...
        if nwp_model_data:
            ...
            self.nwp_model_data = settings.settings['paths']['NWP_model_database']
    ...
    def process_calculations(self):
        ...
        for t, data_str, data_str_corrected in zip(total_time_delays, data_strs, data_strs_corrected):
            ...
            adjusted_data['geo_height'] = adjust_time.model_data['geo_height']
