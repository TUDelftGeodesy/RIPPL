...
class CreateSwathXmlRes():
    ...
    def __init__(self, swath_xml='', swath_etree=''):
        ...
        if swath_etree:
            if not isinstance(swath_etree, etree._ElementTree):
                logging.info('swath_etree should be an instance of the etree class!')
                return
            else:
    ...
    def __call__(self, orbit_class):
        ...
        for readfiles, crop, coverage in zip(self.burst_readfiles, self.burst_crop, self.burst_coverage):
            ...
            resfile.add_readfile(readfiles)
            resfile.add_orbit(self.swath_orbit)
            resfile.create_header()
    ...
    def create_swath_orbit(self, orbit_class):
        ...
        orbit_type = 'precise'
        orbit_dat = orbit_class.interpolate_orbit(orbit_time, orbit_type, satellite='S' + sat)
        if orbit_dat == False:
    ...
    def burst_swath_coverage(self):
        ...
        for i in np.arange(self.swath_readfiles['Number_of_bursts']):
            ...
            if len(unique_lines) > n + 1:
                ...
                logging.info('Invalid burst coordinates found in .xml data ' + self.swath_xml)
