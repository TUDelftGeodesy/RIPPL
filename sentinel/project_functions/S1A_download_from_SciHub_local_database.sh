...
if [ "$#" -eq 0 ] ; then
    ...
    echo " --projectfolder= \"/DESTINATION_FOLDER/\""
	echo " "
...
#                                echo Polarisation Mode  assigned
			...
			if [ "$param" == "projectfolder"  ] ; then
				if [ ! -z "$value" ] ; then
					if [ ! -d $value ] ; then
						echo "Creating folder $value"
						mkdir -p $value
					fi
					echo "Changing projectfolder directory : $value"
					project=$value
				fi
			fi
...
#  Saving metadata files and create symbolic link.
		...
		if [ ! -f ${project}/${year}${month}${day}_${hour}${minute}/sentinel/$name.xml ] ; then
			if [ ! -e ${project}/${year}${month}${day}_${hour}${minute} ] ; then
				mkdir -p ${project}/${year}${month}${day}_${hour}${minute}
			fi
			if [ ! -e ${project}/${year}${month}${day}_${hour}${minute}/sentinel ] ; then
				mkdir -p ${project}/${year}${month}${day}_${hour}${minute}/sentinel
			fi

			cp ${file} ${project}/${year}${month}${day}_${hour}${minute}/sentinel/${name}.xml
        ...
        # If file should be downloaded.
		if [  "$downloadflag" == "true" ] ; then
			if [ ! -f ${area}_s1_${direction}_t${track}/${dataset}_${polmode}/${year}${month}${day}/${name}.zip ] ; then
				echo "Downloading ${name}.zip in folder ${area}_s1_${direction}_t${track}/${dataset}_${polmode}/${year}${month}${day}" >> downloading.log

				# If the folder does not exist where file is downloaded
